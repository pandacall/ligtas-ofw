...
describe("scanPost — verdict-cases.md P1-P7", () => {
  it("P1 (scam-01): a single critical flag, no agency name => HIGH_RISK, no registry lookup", async () => {
    ...
    const result = await scanPost({ kind: "text", text: "Processing fee lang na ₱15,000 para sa medical." }, {
      extractor,
  ...
  it("P2 (scam-03): trafficking corridor destination alone => HIGH_RISK", async () => {
    ...
    const result = await scanPost({ kind: "text", text }, { extractor, registryState: loadFixtureRegistryState() });
    expect(result).toMatchObject({ kind: "scored", verdict: "HIGH_RISK" });
  });
  ...
  it("P3 (scam-15 impersonation): registry VERIFIED + post HIGH_RISK => combined HIGH_RISK (worst-of), demoable end-to-end", async () => {
    ...
    const result = await scanPost({ kind: "text", text }, { extractor, registryState: loadFixtureRegistryState(), now: new Date("2026-07-27T00:00:00.000Z") });
    expect(result).toMatchObject({ kind: "scored", verdict: "HIGH_RISK" });
    if (result.kind === "scored") {
  ...
  it("P7: extraction schema parse fails twice => unanalyzable", async () => {
    ...
    const result = await scanPost({ kind: "text", text: "some post text" }, { extractor, registryState: loadFixtureRegistryState() });
...
const FAKE_SCREENSHOT_DATA_URL =
  "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=";

describe("scanPost — image input (ADR-0003)", () => {
  it("does NOT drop a red flag with a fabricated evidence_quote for image input (no substring guard on screenshots)", async () => {
    const extractor = fakeExtractor([
      baseExtraction({
        red_flags: [{ flag: "upfront_fee", evidence_quote: "this quote never appeared anywhere" }],
      }),
    ]);
    const result = await scanPost(
      { kind: "image", dataUrl: FAKE_SCREENSHOT_DATA_URL },
      { extractor, registryState: loadFixtureRegistryState() },
    );
    expect(result).toMatchObject({ kind: "scored" });
    if (result.kind === "scored") {
      expect(result.post.flags).toHaveLength(1);
    }
  });

  it("sends the image as an image_url content-part alongside a text instruction, not as raw text", async () => {
    const extractor = fakeExtractor([baseExtraction({})]);
    await scanPost(
      { kind: "image", dataUrl: FAKE_SCREENSHOT_DATA_URL },
      { extractor, registryState: loadFixtureRegistryState() },
    );
    const userMessage = extractor.calls[0]?.[1];
    ...
    expect(parts).toContainEqual({ type: "image_url", image_url: { url: FAKE_SCREENSHOT_DATA_URL } });
  });

  it("screenshot fixture (scam-style): image input still produces HIGH_RISK Combined Verdict end-to-end", async () => {
    const extractor = fakeExtractor([
      baseExtraction({
        agency_name: XYZ_AGENCY_NAME,
        position: "nurse",
        destination_country: "UK",
        red_flags: [
          { flag: "page_hijack_pattern", evidence_quote: "bagong Facebook page daw" },
          { flag: "avoid_official_contact_instruction", evidence_quote: "wag na tumawag sa landline" },
        ],
      }),
    ]);
    const result = await scanPost(
      { kind: "image", dataUrl: FAKE_SCREENSHOT_DATA_URL },
      { extractor, registryState: loadFixtureRegistryState(), now: new Date("2026-07-27T00:00:00.000Z") },
    );
    expect(result).toMatchObject({ kind: "scored", verdict: "HIGH_RISK" });
    if (result.kind === "scored") {
      expect(result.registry?.verdict).toBe("VERIFIED");
  ...
  it("escape-hatch parity: not_a_job_post for image input, same as text input", async () => {
    const extractor = fakeExtractor([baseExtraction({ is_job_post: false, agency_name: null })]);
    const result = await scanPost(
      { kind: "image", dataUrl: FAKE_SCREENSHOT_DATA_URL },
      { extractor, registryState: loadFixtureRegistryState() },
  ...
  it("escape-hatch parity: unanalyzable for image input when extraction fails validation twice, same as text input", async () => {
    const extractor = fakeExtractor([{ not_even_close_to_the_schema: true }, { still_wrong: true }]);
    const result = await scanPost(
      { kind: "image", dataUrl: FAKE_SCREENSHOT_DATA_URL },
      { extractor, registryState: loadFixtureRegistryState() },
