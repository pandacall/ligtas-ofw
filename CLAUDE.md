...
- `starter/fixtures-posts.json` is the **LLM eval set** — the fixture data and the verdict-level accuracy metric (`_readme`) are defined, and the CI gate is now wired (issue #10): `packages/core/src/eval.ts` grades the real Extractor + real verdict engine against the pre-registry post verdict, `npm run eval` (`scripts/eval-fixtures.ts`) runs it at temperature 0, and `.github/workflows/eval.yml` gates PRs that touch the extraction/verdict code paths at the ≥90% bar (skippable via the `skip-eval` label). The actual measured accuracy for the default free model and the resulting few-shot decision are not yet recorded — that requires a live `EXTRACTOR_API_KEY` (see issue #10) and is the next step once one is available. Scams use synthetic names only (no real agency defamed). Add real cases from DMW advisories over time.
